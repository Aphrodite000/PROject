...
public class FileScanner {
    ...
    private void doScan(File dir){
        //callback.callback(dir);//如果是文件的话，在这一步就被保存了
        ...
        pool.execute(new Runnable() {//3.线程池执行的任务类
            ...
            public void run() {
                try{
                    ...
                    //如果放在外面，scan方法要阻塞的，等待里面的线程完了才能返回
                    //就执行不了waitfinish，如果放进来，开启多线程执行以后，scan方法
                    //就可以返回了，就可以执行到下面的阻塞方法一直阻塞，就可以被中断
                    ...
                    if(children!=null){
                        for(File child:children){
                            if(child.isDirectory()){
                                ...
                                //每次递归启动新任务在线程池运行
                                //这块有问题，这相当于是任务里面有启动了子任务，只是把任务提交到线程池，
                                // 可是原本的任务也还在执行，还在跑，直到没有子任务，线程也就不能处理完当前任务在去处理后面的，
                                //线程一直卡在原本的任务那里，任务嵌套子任务一直在执行，因为递归所以第一个就结束不了

                                //第一个线程递归调用，然后第二个线程进来，然后递归 调用第三个线程
                                //线程把一段代码执行完毕，才能去执行新的任务，可是这里在递归，任务没办法结束
                                //线程一直得不到返回
                            }
                        }
                    }
    ...
    public void waitFinish() throws InterruptedException {
        ...
        try {
            ...
            //线程池要运行一个线程，要穿进去一个runnable的任务类
            //但线程池内部对运行的线程有自己的封装，相当于自己封装了一个Thread类来进行线程的包装
            //线程类内部里面可以做很多事情

            //两种关闭线程池的方式   都是通过通过线程内部的interrupt()来中断
            //pool.shutdown();
            pool.shutdownNow();//内部实现原理通过内部thread.stop()来停止线程  不安全
            //新的任务线程池就不处理了，由这一行代码来处理任务
            //相当于这一行代码里面，自己把所有的任务都处理掉，这就是不能中断的原因
            //所以要改变方式
        }
    }
